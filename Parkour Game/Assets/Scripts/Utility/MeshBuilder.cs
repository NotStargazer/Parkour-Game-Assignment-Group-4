...
namespace Utility
{
    public ref struct MeshBuilder
    {
        ...
        public static Mesh CreateUVScaledSlopeMesh(Vector3 scale, float thickness, string name)
        {
            var half = Vector3.one * 0.5f;
            var offset = thickness / scale.y;
            scale *= 0.5f;
            thickness *= 0.5f;
            ...
            var ubl = new Vector3(-half.x, -half.y + offset, -half.z);
            var ufl = new Vector3(-half.x, -half.y + offset,  half.z);
            var lbr = new Vector3( half.x,  half.y - offset, -half.z);
            var lfr = new Vector3( half.x,  half.y - offset,  half.z);
            ...
            rhs.y = ufl.y - ubl.y;
            rhs.z = ufl.z - ubl.z;
            var slopeNormal = Vector3.Cross(in lhs, in rhs).normalized;
            var slopeHypotenuse = Vector3.Distance(ubl, ubr);
            
            meshBuilder.AddCubePlane(Vector3.back,    new Vector2(scale.x, thickness),       lbl, ubl, ubr, lbr);
            meshBuilder.AddCubePlane(Vector3.forward, new Vector2(scale.x, thickness),       lfr, ufr, ufl, lfl);
            meshBuilder.AddCubePlane(Vector3.left,    new Vector2(scale.z, thickness),       lfl, ufl, ubl, lbl);
            meshBuilder.AddCubePlane(Vector3.right,   new Vector2(scale.z, thickness),       lbr, ubr, ufr, lfr);
            meshBuilder.AddCubePlane(slopeNormal,     new Vector2(slopeHypotenuse, scale.z), lfl, lbl, lbr, lfr);
            meshBuilder.AddCubePlane(-slopeNormal,    new Vector2(slopeHypotenuse, scale.z), ubl, ufl, ufr, ubr);
